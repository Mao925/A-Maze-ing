#!/usr/bin/env python3
"""設定ファイルから迷路を作る、A-Maze-ing の入口プログラム。

このファイルは担当者Bの担当です。迷路そのものを作る処理は
``mazegen.MazeGenerator`` に任せ、このファイルでは次の仕事だけをします。

* 人が書いた config.txt を読み、型を確認する
* 生成器を呼び、結果をファイルへ保存する
* 端末で読みやすい ASCII 迷路を表示する

初学者向けの小さな関数に分けているので、設定項目や表示を変えたいときも
変更場所を見つけやすくなっています。
"""
